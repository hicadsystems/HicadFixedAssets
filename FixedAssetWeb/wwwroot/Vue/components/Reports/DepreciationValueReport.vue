...
export default {
  ...
  methods: {
    sortingProcess() {
      axios
        .post(
          `/api/GenerateDepreciation/sortAssetDepreciation/`,
          this.sortDepreciation
        )

        .then((response) => {
          if (response.data.responseCode === 404) {
            this.$alert(
              `No Value available for the selected Dates!!`,
              "No Records Found",
              "Warning"
            );
          }
    ...
    generateReport() {
      let startDay = moment(this.sortDepreciation.startDate).format(
        "MM DD YYYY, h:mm:ss a"
      );

      let endDay = moment(this.sortDepreciation.endDate).format(
        "MM DD YYYY, h:mm:ss a"
      );
